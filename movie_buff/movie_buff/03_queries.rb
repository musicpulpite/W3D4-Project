...
def costars(name)
  ...
  Actor
    .joins(:castings)
    .where('actors.name != ? AND castings.movie_id IN (?)', name,
        Movie
          .select(:id)
          .joins(:actors)
...
def actor_out_of_work
  ...
  subquery = Actor.select('actors.id').joins(:castings).group('actors.id').having('COUNT(castings.movie_id) >= 1')
  Actor
    .select(:name)
    .where.not(actors: { id: subquery })
    .length
  end
